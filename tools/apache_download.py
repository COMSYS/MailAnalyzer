...
archive_overview_regex = re.compile(r'.*<li><a\s+(?:[^>]*?\s+)?href=\'([^\']*)\'')
mbox_overview_regex = re.compile(r'.*<a\s+(?:[^>]*?\s+)?href=\"([^\"]*\.mbox)/thread\"')
...
def downloadFile(link, filename, retry=retry_count):
	...
	try:
		...
		print ' * Failed to download: urlretrieve returned {}.'.format(e)
		if retry > 0:
